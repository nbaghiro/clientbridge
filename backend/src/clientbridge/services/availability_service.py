...
async def open_windows(
    ...
    """The staff member's open work intervals (UTC wall-clock) on `on_date`.
...
async def is_within_availability(
    ...
    """Whether ``[start, end]`` (UTC) sits inside an open window on its date, or the day is open."""
    start_utc, end_utc = _as_utc(start), _as_utc(end)
    windows = await open_windows(db, staff_id, business_id, start_utc.date())
    if windows is None:
        return True
    if end_utc.date() != start_utc.date():
        return False
    return any(ws <= start_utc.time() and end_utc.time() <= we for ws, we in windows)
